...
<script type="text/javascript">
    ...
    function _go() {
        ...
        location = "${pb.url}&page.currentPage=" + pc;
    }
</script>
...
<div class="divBody">
    <div class="divContent">
        ...
        <c:choose>
            ...
            <c:otherwise>
                <a href="${pb.url }&page.currentPage=${pb.currentPage-1}" class="aBtn bold">上一页</a>
            </c:otherwise>
        </c:choose>
        ...
        <c:choose>
            <c:when test="${pb.pageCount <= 6 }">
                ...
                <c:set var="end" value="${pb.pageCount }"/>
            </c:when>
            <c:otherwise>
                ...
                <c:if test="${end > pb.pageCount }">
                    <c:set var="begin" value="${pb.pageCount-5 }"/>
                    <c:set var="end" value="${pb.pageCount }"/>
                </c:if>
            </c:otherwise>
        </c:choose>
        ...
        <c:forEach begin="${begin }" end="${end }" var="i">
            <c:choose>
                ...
                <c:otherwise>
                    <a href="${pb.url }&page.currentPage=${i}" class="aBtn">${i }</a>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        <%-- 计算begin和end --%>
        ...
        <%--下一页 --%>
        <c:choose>
            <c:when test="${pb.currentPage eq pb.pageCount }">
                <span class="spanBtnDisabled">下一页</span>
            </c:when>
            <c:otherwise>
                <a href="${pb.url }&page.currentPage=${pb.currentPage+1}" class="aBtn bold">下一页</a>
            </c:otherwise>
        </c:choose>
